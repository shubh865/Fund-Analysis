...
const SOURCE_URL = 'https://www.icicipruamc.com/media-center/downloads?currentTabFilter=Disclosures&subCatTabFilter=MonthlyPortfolioDisclosures';
...
const portfoliosByFamily = new Map(portfolios.map((portfolio) => [family(portfolio.name), portfolio]));
...
const mapped = db.transaction(() => schemes.reduce((count, scheme) => {
  const portfolio = portfoliosByFamily.get(family(scheme.name));
  if (!portfolio) return count;
  upsert.run(scheme.scheme_code, portfolio.portfolio_id, SOURCE_URL);
  return count + 1;
}, 0))();

console.log(`Provisionally mapped ${mapped} ICICI Growth plans to monthly disclosure portfolios by scheme family.`);
